...
public class ServerSide implements Runnable{
     ...
     */
    ServerSide(CollectionManager serverCollection, Socket incoming, ObjectOutputStream who, ObjectInputStream from) {
    ...
    public void run() {
        try {
            authUser(sendToClient, getFromClient, incoming);
            ServerConnection.tellSpecific("Соединение установлено.\nВы можете вводить команды. Ваш поток: "+Thread.currentThread().getId(), sendToClient);
            ...
            while (true) {
                try {
                    ...
                    if (parsedCommand.length == 1) {
                        ServerConnection.tellSpecific(availableCommands.getOrDefault(parsedCommand[0], errorCommand).execute(), sendToClient);
//                        sendToClient.writeObject(availableCommands.getOrDefault(parsedCommand[0], errorCommand).execute());
                        ServerConnection.tellEveryone("от тебя говной воняет");
                        ...
                        ServerConnection.tellSpecific(availableCommands.getOrDefault(parsedCommand[0], errorCommand).execute(parsedCommand[1]), sendToClient);
                        serverCollection.addToHistory(parsedCommand[0]);
                    }
    ...
    public static void authUser(ObjectOutputStream sendToClient, ObjectInputStream getFromClient, Socket incoming){
        try {
            ServerConnection.tellSpecific("Для подключения к БД, введите действущий логин и пароль:" +
                    "\nlogin <user> <password>"+
